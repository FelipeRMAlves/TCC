...
dt = 0.1          # time step
nIter = 1000      # numero de iteracoes
...
Ly = 0.025
Lz = 0.1
le = 0.0014        # tamanho medio do elemento
...
T = 10*np.ones((npoints), dtype='double')     # Temperaturas
...
df = pd.DataFrame([1000*X, 1000*Y, 1000*Z, T_time[0], T_time[100], T_time[200], T_time[300], 
                    T_time[400], T_time[500], T_time[750], T_time[-1]]).T


df.to_excel(f'TempV2.xlsx',
            header=header,
            float_format="%.6f",
            index=False
            )
